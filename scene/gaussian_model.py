...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.inverse_opacity_activation = lambda x: inverse_sigmoid(x)
        self.rotation_activation = lambda x: torch.nn.functional.normalize(x)

        if not self.use_sh:
            # position activation
            self.pos_activation = lambda x: torch.tanh(x)

            # feature activation is individual for each type of color model

            # weight activation
            if self.color_size == 3:
                self.weight_activation = lambda x: torch.tanh(x) 
            if self.color_size == 1:
                self.weight_activation = lambda x: torch.sigmoid(x)


    def __init__(self, sh_degree : int, use_sh: bool, params_size :int, position_size :int, color_size :int):
        self.use_sh = use_sh
    ...
    def get_features(self):
        ...
        if self.use_sh:
            ...
            features_rest = torch.cat((self.get_features_pos, self._features_shape, self.get_features_weight), dim=1)
    ...
    @property
    def get_features_pos_act(self):
        return self._features_pos[:,:,0:self.position_size * self.active_sh_degree]
    @property
    def get_features_shape_act(self):
        return self._features_shape[:,:,0:(self.params_size - self.position_size - self.color_size) * self.active_sh_degree]
    @property
    def get_features_weight_act(self):
        return self._features_weight[:,:,0:self.color_size * self.active_sh_degree]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.use_sh:
            ...
            self._features_dc = nn.Parameter(features[:,:,-3:].transpose(1, 2).contiguous().requires_grad_(True))
            self._features_pos = nn.Parameter(features[:,:,pos_start:pos_end].transpose(1, 2).contiguous().requires_grad_(True))
            self._features_shape = nn.Parameter(features[:,:,shape_start:shape_end].transpose(1, 2).contiguous().requires_grad_(True))
            self._features_weight = nn.Parameter(features[:,:,weights_start:weights_end].transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        if self.use_sh:
            ...
            l = [
                {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
                {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
                {'params': [self._features_pos], 'lr': f_lr, "name": "f_pos"},
                {'params': [self._features_shape], 'lr': f_lr, "name": "f_shape"},
                {'params': [self._features_weight], 'lr': f_lr, "name": "f_weight"},
                {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
                {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
                {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"}
            ]
        ...
        # Custom cosine schedule (applies only to non-xyz groups)
        def lr_lambda(step):
    ...
    def get_feature_pos_lr(self):
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "f_pos":
    ...
    def save_ply(self, path):
        ...
        if self.use_sh:
            ...
            f_rest =  torch.cat((self._features_pos, self._features_shape, self._features_weight), dim=1).detach().flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def load_ply(self, path):
        ...
        if self.use_sh:
            ...
            features_extra = np.zeros((xyz.shape[0], (self.max_sh_degree) * self.params_size))
            for idx, attr_name in enumerate(extra_f_names):
                features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
            # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        ...
        if self.use_sh:
            ...
            self._features_pos = nn.Parameter(torch.tensor(features_extra[:,:,0:self.position_size*self.max_sh_degree], dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
            self._features_shape = nn.Parameter(torch.tensor(features_extra[:,:,self.position_size*self.max_sh_degree:-self.color_size *self.max_sh_degree], dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
            self._features_weight = nn.Parameter(torch.tensor(features_extra[:,:,-self.color_size * self.max_sh_degree:], dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        if self.use_sh:
            ...
            self._features_pos = optimizable_tensors["f_pos"]
            self._features_shape = optimizable_tensors["f_shape"]
            self._features_weight = optimizable_tensors["f_weight"]
        self._opacity = optimizable_tensors["opacity"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, reset_params=True):
        if self.use_sh:
            ...
            d = {"xyz": new_xyz,
            "f_dc": new_features_dc,
            "f_pos": new_features_rest[0],
            "f_shape": new_features_rest[1],
            "f_weight": new_features_rest[2],
            "opacity": new_opacities,
            "scaling" : new_scaling,
            "rotation" : new_rotation}
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if self.use_sh:
            ...
            new_features_rest = [self._features_pos[selected_pts_mask].repeat(N,1,1), self._features_shape[selected_pts_mask].repeat(N,1,1), self._features_weight[selected_pts_mask].repeat(N,1,1)]
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        if self.use_sh:
            ...
            new_features_rest = [self._features_pos[selected_pts_mask], self._features_shape[selected_pts_mask], self._features_weight[selected_pts_mask]]
    ...
    def replace_tensors_to_optimizer(self, inds=None):
        if self.use_sh:
            ...
            tensors_dict = {"xyz": self._xyz,
                "f_dc": self._features_dc,
                "f_pos": self._features_pos,
                "f_shape": self._features_shape,
                "f_weight": self._features_weight,
                "opacity": self._opacity,
                "scaling" : self._scaling,
                "rotation" : self._rotation}
        optimizable_tensors = {}
    ...
    def _update_params(self, idxs, ratio):
        ...
        if self.use_sh:
            ...
            return self._xyz[idxs], self._features_dc[idxs], self._features_pos[idxs], self._features_shape[idxs], self._features_weight[idxs], new_opacity, new_scaling, self._rotation[idxs]
    ...
    def add_new_gs(self, cap_max):
        ...
        if self.use_sh:
            ...
            new_features_rest = [new_features_pos, new_features_shape, new_features_weight]
